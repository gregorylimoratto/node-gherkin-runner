...
(function(){
	...
	function generateExample(header, example, step) {
        for (var i = 0; i < example.cells.length; i++) {
            ...
            if (step.argument && step.argument.type === "DataTable" && step.argument.rows.length > 1) {
                for (var j = 1; j < step.argument.rows.length; j++) {
                        for (var k = 0; k < step.argument.rows[j].cells.length; k++) {
                            step.argument.rows[j].cells[k].value = step.argument.rows[j].cells[k].value.replace(re, value);
                        }
                }
            }
        }
